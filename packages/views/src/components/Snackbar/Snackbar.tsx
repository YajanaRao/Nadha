...
export const Snackbar = ({ icon, onPress, title, subtitle }: SnackbarProps) => {
  ...
  return (
    <Card style={[SNACK_CONTAINER, { backgroundColor }]}>
      ...
      <TouchableOpacity style={SNACK_PRESSABLE} onPress={onPress}>
        <Text style={[SNACK_TITLE, { color: textColor }]}>{title}</Text>
        {subtitle ? (
          <Text style={{ color: descriptionColor }}>{subtitle}</Text>
        ) : (
          false
        )}
      </TouchableOpacity>
